...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, sig=1):
    ...
    def forward(self, x):
        ...
        out_2 = out_2.reshape(out_2.size(0),int(math.sqrt(self.num_classes)),int(math.sqrt(self.num_classes)))
        out_2 += self.sig * torch.eye(int(math.sqrt(self.num_classes))).cuda(out_2.device)
...
class ResNet_N(nn.Module):
    ...
    def forward(self, x):
        ...
        out_1 = out.view(out.size(0), -1)
        out_2 = self.bayes_linear(out_1)
        out_2 = out_2.reshape(out_2.size(0),self.num_classes)
        out_2 = F.softmax(out_2, dim=-1)
        
        return out_2.float()

def ResNet18_N(num_classes):
    return ResNet_N(BasicBlock, [2,2,2,2], num_classes)

def ResNet18_F(num_classes):
    return ResNet_F(BasicBlock, [2,2,2,2], num_classes)
        
def ResNet18(num_classes, sig=1):
    return ResNet(BasicBlock, [2,2,2,2], num_classes, sig=sig)
